...
-- 인라인 뷰 : from 절 뒤에 쓰여지는 서브 쿼리 -> 가상 테이블
...
-- as : 서브쿼리를 이용해서 기존 테이블의 구조로 생성, 데이터 저장 제약조건은 카피되지 않는다!!
create table emp02
as
select * from emp;
...
alter table emp01 modify (job varchar(30) default 'manager') ;
...
-- 테이블의 이름변경 : rename 기존테이블이름 to 새로운 이름
select * from emp03;
select * from test03;
...
create table emp01 (
    ...
    sal number(7,2) constraint emp01_sal_sk check(sal between 500 and 5000), -- 500~5000
    gender char(1) constraint emp01_gender_gk check(gender in('M', 'F')), -- 남자 M, 여자 F
    deptno number(2) constraint emp01_deptno_fk references dept(deptno)
);
...
insert into emp01(empno, ename, sal, gender, job, deptno) values(1111, 'KING', 5000, 'M', 'MANAGER', 10);
insert into emp01(empno, ename, sal, gender, job, deptno) values(1111, 'KING', 5000, 'M', 'MANAGER', 10, sysdate);
select * from emp01;